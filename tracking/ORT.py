...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='baseline_rephead_4_lite_search5',
...
class STARK(nn.Module):
    ...
    def forward(self, img: torch.Tensor, mask: torch.Tensor,
        ...
        print(f"feat_vec_z size: {feat_vec_z.size()}")
        print(f"feat_vec_x size: {feat_vec_x.size()}")

        # Resize feat_vec_z and pos_vec_z to match feat_vec_x
        feat_vec_z = F.interpolate(feat_vec_z.permute(1, 2, 0), size=feat_vec_x.size(0)).permute(2, 0, 1)
        pos_vec_z = F.interpolate(pos_vec_z.permute(1, 2, 0), size=feat_vec_x.size(0)).permute(2, 0, 1)
        mask_vec_z = F.interpolate(mask_vec_z.unsqueeze(1).float(), size=feat_vec_x.size(0)).squeeze(1).to(torch.bool)

        print(f"Resized feat_vec_z size: {feat_vec_z.size()}")
        print(f"Resized pos_vec_z size: {pos_vec_z.size()}")
        print(f"Resized mask_vec_z size: {mask_vec_z.size()}")
        ...
        k = feat_vec + pos_vec
        v = feat_vec
        key_padding_mask = torch.cat([mask_vec_z, mask_vec_x], dim=1)  # Concatenate mask_vec_z and mask_vec_x

        # Ensure the dimensions match
        print(f"Query size before linear: {q.size()}")
        print(f"Key size before linear: {k.size()}")
        print(f"Value size before linear: {v.size()}")

        q = self.transformer.encoder.norm(q)
        k = self.transformer.encoder.norm(k)
        v = self.transformer.encoder.norm(v)
        ...
        fx_t = fx.view(1, fx.shape[1], self.feat_sz_s, self.feat_sz_s).contiguous()
        # run the corner head
...
if __name__ == "__main__":
    ...
    # Debugging information
    print(f"img_x size: {img_x.size()}")
    print(f"mask_x size: {mask_x.size()}")
    print(f"feat_vec_z size: {feat_vec_z.size()}")
    print(f"mask_vec_z size: {mask_vec_z.size()}")
    print(f"pos_vec_z size: {pos_vec_z.size()}")
    ...
    torch.onnx.export(torch_model,  # model being run
                      ...
                      opset_version=11,  # Keeping ONNX version to 11 to avoid einsum
