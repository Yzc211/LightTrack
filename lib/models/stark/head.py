...
def conv(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1,
    ...
    if freeze_bn:
        return nn.Sequential(
            ...
            FrozenBatchNorm2d(out_planes),  # diff
...
class Corner_Predictor_Lite(nn.Module):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16):
        ...
        self.feat_sz = feat_sz  # feature size？ 特征图尺寸
        self.stride = stride
        self.img_sz = self.feat_sz * self.stride  # 图像尺寸
        '''convolution tower for two corners 两个角点'''
        '''这个卷积塔的作用是逐步降低特征图的通道数，同时提取更高级的特征。最终，它输出一个通道数为 2 的特征图，通常用于预测两个角点的位置或其他属性'''
        ...
        with torch.no_grad():
            ...
            # self.indice——特征图的索引在0原始图像空间中中心的位置
            # generate mesh-grid
            self.coord_x = self.indice.repeat((self.feat_sz, 1)) \
                .view((self.feat_sz * self.feat_sz,)).float().cuda()
            self.coord_y = self.indice.repeat((1, self.feat_sz)) \
                .view((self.feat_sz * self.feat_sz,)).float().cuda()
            '''本段代码生成了一个特征图对应的二维坐标网格，其中：self.coord_x 表示特征图中每个单元的 x 坐标 self.coord_y 表示特征图中每个单元的 y 坐标。
            这些坐标是基于特征图的索引和步长映射到原始图像空间中的'''

    def forward(self, x, return_dist=False, softmax=True):
        """ Forward pass with input x. """
        '''tl:top-left br:bottom-right'''
        score_map_tl, score_map_br = self.get_score_map(x)  # 计算得分图

        '''return_dist=True 表示要返回概率分布向量'''
        if return_dist:
            coorx_tl, coory_tl, prob_vec_tl = self.soft_argmax(score_map_tl, return_dist=True, softmax=softmax)
            coorx_br, coory_br, prob_vec_br = self.soft_argmax(score_map_br, return_dist=True, softmax=softmax)
            return torch.stack((coorx_tl, coory_tl, coorx_br, coory_br), dim=1) / self.img_sz, prob_vec_tl, prob_vec_br
...
class Corner_Predictor_Lite_Rep(nn.Module):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16):
        ...
        '''RepVGGBlock: train:3*3+1*1+identity inference:3*3+Relu'''
